  ...
  .then(() => {
    ...
    return Promise.resolve()
      .then(() => {
        ...
        if (env.user) {
          _.merge(headers, {
            'Authorization': basicAuthHeader(env.user, env.password)
          })
        }
        const transportOpt = {headers}
        const client = new Lokka({transport: new Transport(env.baseurl, transportOpt)})
        if (argv.fragment) {
        ...
        if (!argv.fragment && data) {
          ...
          return client[argv.mutation ? 'mutate' : 'query'](queryData, vars).then(result => {
